package rs.craby.crabymoduletest
...
class CrabyModuleTestPackage : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyCrabyModuleTest_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-craby-module-test")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return CrabyModuleTestPackage.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
